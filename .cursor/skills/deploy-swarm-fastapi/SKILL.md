...
| OTEL | `OTEL_APPEND_IP_SUFFIX=False` no `.env` do servidor |
...
Repassar `OTEL_ENABLED`, `OTEL_APPEND_ENV`, `OTEL_EXPORTER_OTLP_ENDPOINT`, `LOKI_URL`, `LOG_LEVEL` no `environment` da stack. Nome no Tempo/Loki: `<base>-<ambiente>` (sem octeto IP).
